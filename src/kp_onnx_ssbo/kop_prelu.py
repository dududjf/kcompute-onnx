...
layout (std430, set = 0, binding = 0) readonly  buffer InBufData  {{ float in_tensor_data[];  }};
layout (std430, set = 0, binding = 1) readonly  buffer InBufSlope {{ float in_tensor_slope[]; }};
...
void main() {{
    ...
    uint size_x_data = params[0];
    uint size_y_data = params[1];
    uint size_z_data = params[2];
    if (gx >= size_x_data || gy >= size_y_data || gz >= size_z_data) return;

    uint size_x_slope = params[3];
    uint size_y_slope = params[4];
    uint size_z_slope = params[5];

    uint stride_y_data = size_z_data;
    uint stride_x_data = size_y_data * stride_y_data;
    uint idx_data = gx * stride_x_data + gy * stride_y_data + gz;

    uint stride_y_slope = size_z_slope;
    uint stride_x_slope = size_y_slope * stride_y_slope;
    uint idx_slope = size_z_slope > 1 ? gz : 0;
    if (size_y_slope > 1) idx_slope += gy * stride_y_slope;
    if (size_x_slope > 1) idx_slope += gx * stride_x_slope;
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        if len(shape_data) == 1:
            ...
            size_x_data = int(np.prod(shape_data[:-2]))
            size_y_data, size_z_data = shape_data[-2], shape_data[-1]
            size_x_slope = int(np.prod(new_shape_slope[:-2]))
            size_y_slope, size_z_slope = new_shape_slope[-2], new_shape_slope[-1]

        # Create output tensor and algorithm
        size = int(np.prod(shape_data))
        tensor_out = self.manager.tensor(np.zeros(size, dtype=np.float32))
        updated_tensors.append(tensor_out)

        params = np.array([size_x_data, size_y_data, size_z_data,
                           size_x_slope, size_y_slope, size_z_slope], dtype=np.uint32)
        ...
        workgroup = (
            (size_x_data + LOCAL_X_3D - 1) // LOCAL_X_3D,
            (size_y_data + LOCAL_Y_3D - 1) // LOCAL_Y_3D,
            (size_z_data + LOCAL_Z_3D - 1) // LOCAL_Z_3D,
        )

        updated_algorithms.append(self.manager.algorithm(
            [tensor_data, new_slope, tensor_out, param_in],
