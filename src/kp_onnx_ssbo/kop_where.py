...
layout (std430, set = 0, binding = 0) readonly  buffer CondBuf {{ float condition[];  }};
layout (std430, set = 0, binding = 1) readonly  buffer XBuf    {{ float x[];          }};
layout (std430, set = 0, binding = 2) readonly  buffer YBuf    {{ float y[];          }};
...
void main() {{
    ...
    uint size_x_c = params[3];
    uint size_y_c = params[4];
    uint size_z_c = params[5];
    uint size_x_x = params[6];
    uint size_y_x = params[7];
    uint size_z_x = params[8];
    uint size_x_y = params[9];
    uint size_y_y = params[10];
    uint size_z_y = params[11];
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        if len(new_shape_c) == 1:
            size_x_c, size_y_c, size_z_c = new_shape_c[0], 1, 1
        elif len(new_shape_c) == 2:
            size_x_c, size_y_c, size_z_c = new_shape_c[0], new_shape_c[1], 1
        else:
            size_x_c = int(np.prod(new_shape_c[:-2]))
        ...
        if len(new_shape_x) == 1:
            ...
            size_x_x = int(np.prod(new_shape_x[:-2]))
            size_y_x = new_shape_x[-2]
            size_z_x = new_shape_x[-1]

        if len(new_shape_y) == 1:
            size_x_y, size_y_y, size_z_y = new_shape_y[0], 1, 1
        elif len(new_shape_y) == 2:
            size_x_y, size_y_y, size_z_y = new_shape_y[0], new_shape_y[1], 1
        else:
            size_x_y = int(np.prod(new_shape_y[:-2]))
        ...
        out_x = max(max(size_x_c, size_x_x), size_x_y)
        out_y = max(max(size_y_c, size_y_x), size_y_y)
        out_z = max(max(size_z_c, size_z_x), size_z_y)

        params = np.array([out_x, out_y, out_z,
                           size_x_c, size_y_c, size_z_c,
                           size_x_x, size_y_x, size_z_x,
                           size_x_y, size_y_y, size_z_y], dtype=np.uint32)
        ...
        workgroup = (
            (out_x + LOCAL_X_3D - 1) // LOCAL_X_3D,
            (out_y + LOCAL_Y_3D - 1) // LOCAL_Y_3D,
            (out_z + LOCAL_Z_3D - 1) // LOCAL_Z_3D,
        )

        updated_algorithms.append(self.manager.algorithm(
            [new_condition, new_x, new_y, tensor_out, param_in],
