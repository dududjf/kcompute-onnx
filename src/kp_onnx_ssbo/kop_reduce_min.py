...
void main() {{
    ...
    float min_val = in_tensor[in_offset];
    in_offset += block_size;
    for (uint i = 1; i < dimension; ++i, in_offset += block_size) {{
        min_val = min(min_val, in_tensor[in_offset]);
    }}

    out_tensor[out_offset] = min_val;
}}
""")

    def __repr__(self):
        return f"ReduceMinOp({self.manager.get_device_properties()['device_name']})"
