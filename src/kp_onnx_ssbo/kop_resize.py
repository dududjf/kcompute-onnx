...
class ResizeOp:
    ...
    if (in_y_f < 0.0 || in_y_f > float(in_h - 1) || in_x_f < 0.0 || in_x_f > float(in_w - 1)) {
        out_tensor[leading_idx * uint(out_h * out_w) + out_y * uint(out_w) + out_x] = extrapolation_f;
        return;
    }"""
        return ""
...
void main() {{
    ...
    // Bounds check
    uint bc_leading = params[0];
    uint bc_out_h = params[1];
    uint bc_out_w = params[2];
    if (leading_idx >= bc_leading || out_y >= bc_out_h || out_x >= bc_out_w) return;
    
    int in_h = int(params[3]);
    int in_w = int(params[4]);
    int out_h = int(params[5]);
    int out_w = int(params[6]);
    float scale_h = uintBitsToFloat(params[7]);
    float scale_w = uintBitsToFloat(params[8]);
    float extrapolation_f = uintBitsToFloat(params[9]);
    float roi_h_start_f = uintBitsToFloat(params[10]);
    float roi_h_end_f = uintBitsToFloat(params[11]);
    float roi_w_start_f = uintBitsToFloat(params[12]);
    float roi_w_end_f = uintBitsToFloat(params[13]);
...
void main() {{
    // params[0]: leading, params[1]: out_h, params[2]: out_w (bounds check)
    // params[3]: in_h, params[4]: in_w, params[5]: out_h, params[6]: out_w
    // params[7]: scale_h (as uint), params[8]: scale_w (as uint)
    // params[9]: extrapolation (as uint)
    // params[10]: roi_h_start (as uint), params[11]: roi_h_end (as uint)
    // params[12]: roi_w_start (as uint), params[13]: roi_w_end (as uint)
    ...
    for (int dy = -radius_h; dy <= radius_h; ++dy) {{
        ...
        for (int dx = -radius_w; dx <= radius_w; ++dx) {{
            ...
            result += w * in_tensor[base + uint(yc * in_w + xc)];
            weight_sum += w;
        }}
    }}
    ...
    out_tensor[leading_idx * uint(out_h * out_w) + out_y * uint(out_w) + out_x] = result;
}}"""
        else:
            # 普通双线性插值
...
void main() {{
    ...
    uint base = leading_idx * uint(in_h * in_w);
    float v00 = in_tensor[base + uint(y0c * in_w + x0c)];
    float v01 = in_tensor[base + uint(y0c * in_w + x1c)];
    float v10 = in_tensor[base + uint(y1c * in_w + x0c)];
    float v11 = in_tensor[base + uint(y1c * in_w + x1c)];
    
    float v = cy0 * (cx0 * v00 + cx1 * v01) + cy1 * (cx0 * v10 + cx1 * v11);
    
    out_tensor[leading_idx * uint(out_h * out_w) + out_y * uint(out_w) + out_x] = v;
}}"""
...
void main() {{
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        # 构建 params 数组
        # params[0-2]: bounds check (leading, out_h, out_w)
        # params[3-13]: in_h, in_w, out_h, out_w, scale_h, scale_w, extrapolation, roi_h_start, roi_h_end, roi_w_start, roi_w_end
        params = np.array([
            leading,  # params[0]: bounds check leading
            out_h,    # params[1]: bounds check out_h
            out_w,    # params[2]: bounds check out_w
            in_h,     # params[3]: in_h
            in_w,     # params[4]: in_w
            out_h,    # params[5]: out_h
            out_w,    # params[6]: out_w
            np.float32(scale_h).view(np.uint32),           # params[7]: scale_h as uint
            np.float32(scale_w).view(np.uint32),           # params[8]: scale_w as uint
            np.float32(self.extrapolation_value).view(np.uint32),  # params[9]: extrapolation as uint
            np.float32(roi_h_start).view(np.uint32),       # params[10]: roi_h_start as uint
            np.float32(roi_h_end).view(np.uint32),         # params[11]: roi_h_end as uint
            np.float32(roi_w_start).view(np.uint32),       # params[12]: roi_w_start as uint
            np.float32(roi_w_end).view(np.uint32),         # params[13]: roi_w_end as uint
        ], dtype=np.uint32)
        ...
        # 计算 workgroup，使用整数天花板除法
        # x -> leading_idx, y -> out_y, z -> out_x
        workgroup = (
            (leading + LOCAL_X_3D - 1) // LOCAL_X_3D,
            (out_h + LOCAL_Y_3D - 1) // LOCAL_Y_3D,
            (out_w + LOCAL_Z_3D - 1) // LOCAL_Z_3D
        )
