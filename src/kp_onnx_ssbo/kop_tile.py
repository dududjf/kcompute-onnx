...
void main() {{
    ...
    uint repeats = params[1];
    if (gx >= group_count || gy >= repeats) return;
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        for i in reversed(range(len(data_shape))):
            ...
            if repeats[i] > 1:
                ...
                group_count = int(np.prod(data_shape[:i])) if i > 0 else 1

                out_array = np.zeros(group_count * out_block_size, dtype=np.float32)
                tensor_out = self.manager.tensor(out_array)

                params = np.array([group_count, repeats[i], in_block_size, out_block_size], dtype=np.uint32)
                ...
                workgroup = (
                    ...
                    (repeats[i] + LOCAL_Y_2D - 1) // LOCAL_Y_2D,
                    1,
                )
        ...
        return [(tensor_out, list(np.array(data_shape) * repeats))]
