...
layout (std430, set = 0, binding = 0) readonly  buffer InBufX  { float X[];  };
layout (std430, set = 0, binding = 1) readonly  buffer InBufW  { float W[];  };
layout (std430, set = 0, binding = 2) readonly  buffer InBufR  { float R[];  };
layout (std430, set = 0, binding = 3) readonly  buffer InBufB  { float B[];  };
layout (std430, set = 0, binding = 4) readonly  buffer InBufH  { float H[];  };
layout (std430, set = 0, binding = 5) buffer OutBufRH { float RH[]; };
layout (std430, set = 0, binding = 6) buffer OutBufZ  { float Z[];  };
layout (std430, set = 0, binding = 7) readonly  buffer UIParams { uint params[]; };
...
void main() {
    ...
    uint hidden_size_param = params[1];
    if (b >= batch_size || j >= hidden_size_param) return;
    
    uint input_size = params[2];
    uint hidden_size = params[3];
    uint x_offset = params[4];
    uint x_stride_b = params[5];
    uint w_dir_offset = params[6];
    uint r_dir_offset = params[7];
    uint b_dir_offset = params[8];
...
layout (std430, set = 0, binding = 2) readonly  buffer InBufR  { float R[];  };
layout (std430, set = 0, binding = 3) readonly  buffer InBufB  { float B[];  };
layout (std430, set = 0, binding = 4) buffer InOutBufH { float H[];  };
layout (std430, set = 0, binding = 5) readonly  buffer InBufRH { float RH[]; };
layout (std430, set = 0, binding = 6) readonly  buffer InBufZ  { float Z[];  };
layout (std430, set = 0, binding = 7) buffer OutBufY  { float Y[];  };
...
void main() {
    ...
    uint x_offset = params[4];
    uint x_stride_b = params[5];
    uint y_offset = params[6];
    uint y_stride_b = params[7];
    uint w_dir_offset = params[8];
    uint r_dir_offset = params[9];
    uint b_dir_offset = params[10];
...
layout (std430, set = 0, binding = 2) readonly  buffer InBufR  { float R[];  };
layout (std430, set = 0, binding = 3) readonly  buffer InBufB  { float B[];  };
layout (std430, set = 0, binding = 4) readonly  buffer InBufH  { float H[];  };       // [2, batch, hidden]
layout (std430, set = 0, binding = 5) buffer OutBufRH { float RH[]; };                // [2, batch, hidden]
layout (std430, set = 0, binding = 6) buffer OutBufZ  { float Z[];  };                // [2, batch, hidden]
layout (std430, set = 0, binding = 7) readonly  buffer UIParams { uint params[]; };
...
void main() {
    ...
    uint input_size = params[2];
    uint hidden_size = params[3];
    uint batch_hidden = params[4];          // batch * hidden (stride for direction)
    uint x_stride_b = params[5];
    uint x_offset_fwd = params[6];
    uint x_offset_bwd = params[7];
    uint w_stride_dir = params[8];          // 3 * hidden * input
    uint r_stride_dir = params[9];          // 3 * hidden * hidden
    uint b_stride_dir = params[10];         // 6 * hidden
...
layout (std430, set = 0, binding = 2) readonly  buffer InBufR  { float R[];  };
layout (std430, set = 0, binding = 3) readonly  buffer InBufB  { float B[];  };
layout (std430, set = 0, binding = 4) buffer InOutBufH { float H[];  };                // [2, batch, hidden]
layout (std430, set = 0, binding = 5) readonly  buffer InBufRH { float RH[]; };        // [2, batch, hidden]
layout (std430, set = 0, binding = 6) readonly  buffer InBufZ  { float Z[];  };        // [2, batch, hidden]
layout (std430, set = 0, binding = 7) buffer OutBufY  { float Y[];  };
layout (std430, set = 0, binding = 8) buffer OutBufYh { float Y_h[]; };                // [2, batch, hidden]
layout (std430, set = 0, binding = 9) readonly  buffer UIParams { uint params[]; };
...
void main() {
    ...
    uint input_size = params[2];
    uint hidden_size = params[3];
    uint batch_hidden = params[4];          // batch * hidden
    uint x_stride_b = params[5];
    uint x_offset_fwd = params[6];
    uint x_offset_bwd = params[7];
    uint y_stride_b = params[8];
    uint y_offset_fwd = params[9];
    uint y_offset_bwd = params[10];
    uint w_stride_dir = params[11];         // 3 * hidden * input
    uint r_stride_dir = params[12];         // 3 * hidden * hidden
    uint b_stride_dir = params[13];         // 6 * hidden
...
layout (std430, set = 0, binding = 3) readonly  buffer InBufB  { float B[];  };
layout (std430, set = 0, binding = 4) buffer InOutBufH { float H[];  };
layout (std430, set = 0, binding = 5) buffer OutBufY  { float Y[];  };
...
layout (std430, set = 0, binding = 4) buffer InOutBufH { float H[];  };                // [2, batch, hidden]
layout (std430, set = 0, binding = 5) buffer OutBufY  { float Y[];  };
layout (std430, set = 0, binding = 6) buffer OutBufYh { float Y_h[]; };                // [2, batch, hidden]
layout (std430, set = 0, binding = 7) readonly  buffer UIParams { uint params[]; };
        ...
        if is_bidirectional:
            ...
            if self.linear_before_reset:
                ...
                for step in range(seq_length):
                    ...
                    # params: [batch_size, hidden_size, input_size, hidden_size, batch_hidden, x_stride_b,
                    ...
                    param_tensor = self.manager.tensor_t(params, kp.TensorTypes.device)
                    updated_tensors.append(param_tensor)
                    self.manager.sequence().record(kp.OpTensorSyncDevice([param_tensor])).eval()
                    
                    updated_algorithms.append(self.manager.algorithm(
                        [tensor_X, tensor_W, tensor_R, tensor_B, tensor_H, tensor_Y, tensor_Y_h, param_tensor],
                ...
                for step in range(seq_length):
                    ...
                    # Phase 1 params: [batch_size, hidden_size, input_size, hidden_size, batch_hidden, x_stride_b,
                    #                  x_offset_fwd, x_offset_bwd, w_stride_dir, r_stride_dir, b_stride_dir]
                    params_p1 = np.array([
                        batch_size, hidden_size,
                        input_size, hidden_size, batch_hidden, x_stride_b, x_offset_fwd, x_offset_bwd,
                        w_stride_dir, r_stride_dir, b_stride_dir
                    ], dtype=np.uint32)
                    param_tensor_p1 = self.manager.tensor_t(params_p1, kp.TensorTypes.device)
                    updated_tensors.append(param_tensor_p1)
                    self.manager.sequence().record(kp.OpTensorSyncDevice([param_tensor_p1])).eval()
                    
                    updated_algorithms.append(self.manager.algorithm(
                        [tensor_X, tensor_W, tensor_R, tensor_B, tensor_H, tensor_RH, tensor_Z, param_tensor_p1],
                        self.shader_bi_phase1,
                        (batch_size, hidden_size, 1),
                    ))
                    
                    # Phase 2 params: [batch_size, hidden_size, input_size, hidden_size, batch_hidden, x_stride_b,
                    #                  x_offset_fwd, x_offset_bwd, y_stride_b, y_offset_fwd, y_offset_bwd,
                    #                  w_stride_dir, r_stride_dir, b_stride_dir]
                    params_p2 = np.array([
                        batch_size, hidden_size,
                    ...
                    param_tensor_p2 = self.manager.tensor_t(params_p2, kp.TensorTypes.device)
                    updated_tensors.append(param_tensor_p2)
                    self.manager.sequence().record(kp.OpTensorSyncDevice([param_tensor_p2])).eval()
                    
                    updated_algorithms.append(self.manager.algorithm(
                        [tensor_X, tensor_W, tensor_R, tensor_B, tensor_H, tensor_RH, tensor_Z, tensor_Y, tensor_Y_h, param_tensor_p2],
        ...
        # ========== UNIDIRECTIONAL PATH (forward or reverse) ==========
            ...
            if self.linear_before_reset:
                ...
                for step in range(seq_length):
                    ...
                    # params: [batch_size, hidden_size, input_size, hidden_size, x_offset, x_stride_b,
                    #          y_offset, y_stride_b, w_dir_offset, r_dir_offset, b_dir_offset]
                    params = np.array([
                        batch_size, hidden_size,
                    ...
                    updated_algorithms.append(self.manager.algorithm(
                        [tensor_X, tensor_W, tensor_R, tensor_B, tensor_H, tensor_Y, param_tensor],
                ...
                for step in range(seq_length):
                    ...
                    # Phase 1 params: [batch_size, hidden_size, input_size, hidden_size, x_offset, x_stride_b,
                    #                  w_dir_offset, r_dir_offset, b_dir_offset]
                    params_p1 = np.array([
                        batch_size, hidden_size,
                        input_size, hidden_size, x_offset, x_stride_b,
                        w_dir_offset, r_dir_offset, b_dir_offset
                    ], dtype=np.uint32)
                    param_tensor_p1 = self.manager.tensor_t(params_p1, kp.TensorTypes.device)
                    updated_tensors.append(param_tensor_p1)
                    ...
                    # Phase 2 params: [batch_size, hidden_size, input_size, hidden_size, x_offset, x_stride_b,
                    #                  y_offset, y_stride_b, w_dir_offset, r_dir_offset, b_dir_offset]
                    params_p2 = np.array([
                        batch_size, hidden_size,
                        input_size, hidden_size, x_offset, x_stride_b, y_offset, y_stride_b,
                    ...
                    updated_algorithms.append(self.manager.algorithm(
                        [tensor_X, tensor_W, tensor_R, tensor_B, tensor_H, tensor_RH, tensor_Z, tensor_Y, param_tensor_p2],
