...
class LeakyReluOp:
    def __init__(self, manager: kp.Manager, alpha=0.01):
...
void main() {{
    ...
    float alpha = uintBitsToFloat(params[1]);
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        alpha_uint = np.uint32(np.float32(self.alpha).view(np.uint32))
        params = np.array([size, alpha_uint], dtype=np.uint32)
