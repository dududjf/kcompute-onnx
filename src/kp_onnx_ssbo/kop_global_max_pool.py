...
void main() 
{{
    uint num_prefix = params[0];
    uint block = params[1];
    uint gx = gl_GlobalInvocationID.x;
    if(gx >= num_prefix) return;
...
""")
    ...
    def __repr__(self):
        ...
        return f"GlobalMaxPoolOp({device_name})"
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        workgroup = ((num_prefix + LOCAL_X_1D - 1) // LOCAL_X_1D, 1, 1)
        params = np.array([num_prefix, block], dtype=np.uint32)
