import kp
import numpy as np
from .shader_utils import compile_source, LOCAL_X_2D, LOCAL_Y_2D, LOCAL_X_1D
...
""")
        ...
        self.compiled_shader_log = compile_source(f"""
#version 450
...
""")
    ...
    def fuse(self, input_tensors: list[tuple[kp.Tensor, list[int]]], updated_algorithms: list[kp.Algorithm],
        ...
        # Apply log to the result
        out_size = int(np.prod([1 if axis_present[i] else shape_in[i] for i in range(len(shape_in))]))
        
        tensor_out_log = self.manager.tensor(np.zeros(out_size, dtype=np.float32))
        updated_tensors.append(tensor_out_log)

        params_log = np.array([out_size], dtype=np.uint32)
        param_log_in = self.manager.tensor_t(params_log, kp.TensorTypes.device)
        self.manager.sequence().record(kp.OpTensorSyncDevice([param_log_in])).eval()

        workgroup_log = ((out_size + LOCAL_X_1D - 1) // LOCAL_X_1D, 1, 1)

        updated_algorithms.append(self.manager.algorithm(
            [tensor_out, tensor_out_log, param_log_in],
            self.compiled_shader_log,
            workgroup_log,
        ))
